...
const ConsultasIdPaciente: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Certeza que deseja cancelar/excluir esta consulta?")) {
      try {
        await remove(id);
      } catch (err) {
        alert("Erro ao excluir consulta. Verifique se você tem permissão ou contate o suporte.");
      }
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <ClipboardList className="hidden md:block" /> Minhas Consultas
          </h1>
          <p className="text-gray-500 mt-1">
            Acompanhe seu histórico e próximas consultas
          </p>
        </div>
        <Link
      ...
      <div className="rounded-xl p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative border border-gray-200 rounded-lg p-2">
            ...
            <input
              type="text"
              placeholder="Buscar por nome do médico ou ID..."
        ...
        {filteredConsultas.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-gray-50/50">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              Você não possui consultas agendadas com os filtros atuais.
            </p>
          </div>
        ) : (
...
export default ConsultasIdPaciente;
